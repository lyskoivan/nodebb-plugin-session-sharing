...
var nbbAuthController = require.main.require(
...
plugin.normalizePayload = async function (payload, callback) {
  ...
  const userFields = await user.getUserFields(mainResUid, [
    "email",
    "leadership",
    "username",
  ]);
  ...
  if (mainResUid) {
    currentUserEmail = userFields.email;
  }
...
plugin.verifyUser = function (token, uid, isNewUser, callback) {
  plugins.fireHook(
    "static:sessionSharing.verifyUser",
    {
      uid: uid,
      isNewUser: isNewUser,
      token: token,
      ...
      function (groups, next) {
        ...
        if (plugin.settings.syncGroupList === "on") {
          join = join.filter((group) =>
            plugin.settings.syncGroups.includes(group)
          );
        }
...
plugin.addMiddleware = function (req, res, next) {
  ...
  meta.blacklist.test(req.ip, function (err) {
    ...
    if (
      ...
      return plugin.process(req.cookies[plugin.settings.cookieName], function (
        err,
        uid
      ) {
        if (err) {
          ...
          return plugins.fireHook(
            "filter:sessionSharing.error",
            {
              error: err,
              uid: uid,
              res: res,
